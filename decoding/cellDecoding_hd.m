...
cell_type2 = 'head_direction_cell';
...
% [~, hd_dir] = loadInfo(cell_type1, cell_type2, session_path); % PC
hd_dir = [behav_time, behav.hdDir{1}];
...
%% functions;
function [cell_sig, hd_dir] = loadInfo(cell_type1, cell_type2, session_temp)
    try
        load([session_temp, filesep, cell_type1, filesep, cell_type2, filesep, cell_type2, '.mat'], cell_type2);
        load([session_temp, filesep, cell_type1, filesep, 'hd_dir.mat'], 'hd_dir');
...
function [fit_loss_PC_mae, predict_loss_PC_mae] = decodeIntegration(spike_timestamps_PC, pos, p_hd, ...
    ...
    for set_i = 1:ceil(1/bin_size)
        ...
        % decode;
        % PC;
        decoder_m = GNBDecoder_hd('p_hd', p_hd, 't_size', 1, 't_smooth', 10);
...
function [fit_loss_PC_mae_shuffle, predict_loss_PC_mae_shuffle] = decodeShuffle(spike_timestamps_PC, pos, p_hd, bin_size)
    ...
    parfor n = 1:shuffle_num
        [fit_loss_PC_mae_shuffle{n,1}, predict_loss_PC_mae_shuffle{n,1}] = ...
            decodeIntegration(spike_timestamps_PC, pos_shuffle{n}, p_hd, bin_size, 0, '', '');
    end
end
