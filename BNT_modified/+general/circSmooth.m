...
%  USAGE
%
%    smoothed = general.circSmooth(data, smooth)
%
%    data           data to smooth
...
% This code is used to smooth data in a circular way.
% Can be used in the smooth of head direction bins.
%
% Modified by Xiang Zhang, April, 2023.

function smoothed = circSmooth(data, smooth)
	...
	if vector
		...
		if smooth(1) == 0
			...
			for i = 1:size(data,1)
				...
				start = d/2+1;
				stop = start + hSize - 1;
				smoothed(i,:) = tmp(:,start:stop); %#ok<AGROW>
			end
		elseif smooth(2) == 0
