...
% 1. add data time type: p.datatime (default: 'msec');
% 2. add output value: map.p;
% 3. position data out of the range is limited to p.limit;
% 4. change the method of binned time;
% 5. bins with occupied time less than 0.1 sec are discarded (set as null value);
% 6. add all original data into output map;
% 7. add mean rate result.
%
...
function map = map(v, z, varargin)
    ...
    checkDatatime = @(x) helpers.isstring(x, 'msec', 's');
    ...
    addRequired(inp, 'v');
    addRequired(inp, 'z');
    addParameter(inp, 'datatime', defaultDatatime, checkDatatime);
    ...
    for p = 1:numPosGroups
        ...
        % limit x & y;
        x(x>limitsX(2)) = limitsX(2);
        x(x<limitsX(1)) = limitsX(1);
        ...
        if ~isempty(y)
            y(y>limitsY(2)) = limitsY(2);
            y(y<limitsY(1)) = limitsY(1);
        ...
        for s = 1:numIter
            ...
            if pointProcess
                ...
                % ni = knnsearch(t, z); % n(ni) = 1;
                % n = histcounts(ni, (1:(length(t)+1)));
            else
            ...
            if isempty(y)
                ...
                map(m).countRaw(time < 0.1) = 0;
                map(m).Nspikes(time < 0.1) = 0;
                ...
                if pointProcess
                    zRaw = map(m).Nspikes ./ (map(m).timeRaw + eps);
                    map(m).zRaw = zRaw;
                else
                    % zRaw contains sum of z(:, 2) elements that fall in a bin
                    zRaw = general.accumulate([x y], z(:, 2), [nBinsX nBinsY])';
                    % divide sum of elements by number of visits to get mean value
                    zRaw = zRaw ./ (map.Nspikes + eps);
                    map(m).zRaw = zRaw;
                end
            ...
            if showBlanks
                ...
                map(m).zRaw_original = map(m).zRaw;
                map(m).countRaw_original = map(m).countRaw;
                map(m).timeRaw_original = map(m).timeRaw;
                map(m).Nspikes_original = map(m).Nspikes;
                map(m).zRaw(selected) = NaN;
                map(m).countRaw(selected) = NaN;
                map(m).timeRaw(selected) = NaN;
                map(m).Nspikes(selected) = NaN;
            end
            ...
            map(m).meanRate = nansum(nansum(map(m).Nspikes)) / (nansum(nansum(map(m).timeRaw))+eps);
